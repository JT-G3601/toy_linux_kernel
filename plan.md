...
每个 M0-M10 里程碑都必须在 `docs/textbook/` 配套一章面向学习者的教科书式
文档。功能实现、验证和教材共同完成后，里程碑才能标记为 `done`。章节至少包含：

- 背景与前置知识：说明机制解决的问题，首次出现的缩写和寄存器必须解释。
- 进入阶段时的机器状态：CPU mode、关键寄存器、内存、设备和交接不变量。
- 实现方法：按真实执行顺序关联代码文件、关键数据结构、地址与错误路径。
- 示意图：至少一张控制流/状态图，以及一张精确的数据或地址布局图/表。
- 可重复验证：命令、期望输出、失败含义，以及至少一个负路径或边界实验。
- 限制与排错：区分教学取舍、尚未实现的功能、真实缺陷和下一阶段接管点。
- 练习与拓展阅读：优先引用处理器/ABI 标准、工具官方手册、Linux/QEMU
  官方文档或上游源码，并介绍相关当前主流实现方案。

章节统一使用 `docs/textbook/TEMPLATE.md`，状态达到 `verified` 后才能作为里程碑
验收证据。修改实现导致图、常量或流程变化时，必须在同一任务中同步教材。
